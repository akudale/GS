...
public partial class search : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                if (!currentUser.Details.is_admin)
                {
                    trGroup.Visible = false;
                    ddlSubUnit.SelectedValue = currentUser.Details.sub_unit.ToString();
                }
                else
                {
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            u = -1;
            su = currentUser.Details.sub_unit;
        }

        gvResults.EmptyDataText = "<h5>" + GetLocalResourceObject("NoResults") + "</h5>";
        var results = Tickets.Search(db, keywords, usr, dtFrom, dtTo, prty, stat, onlyOpen, u, su,txtMembershipCode.Text);
